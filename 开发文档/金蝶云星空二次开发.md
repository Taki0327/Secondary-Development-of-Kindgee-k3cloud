...
public override void EntityRowDoubleClick(EntityRowClickEventArgs e)
{
    ...
    if (field.Contains("单"))
    {
        switch (field)
        {
            case "销售订单编号": util.OpenEntity(field, "SAL_SaleOrder"); break;
            case "生产订单编号": util.OpenEntity(field, "PRD_MO"); break;
            case "采购申请单编号": util.OpenEntity(field, "PUR_Requisition"); break;
            case "采购订单": util.OpenEntity(field, "PUR_PurchaseOrder"); break;
            case "采购入库单编号": util.OpenEntity(field, "STK_InStock"); break;
            case "收料通知单编号": util.OpenEntity(field, "PUR_ReceiveBill"); break;
        }
    }
}
...
public void OpenEntity(string field, string formid)
{
    try
    {
        //获取行号
        ...
        string data = Convert.ToString(this.Model.GetValue(field, row));
        if (data != "")
        {
            //将订单编号转换为id
...
, OQLFilter.CreateHeadEntityFilter($"FBILLNO='{data}'"));
    ...
    {
        this.View.ShowErrMessage("该数据暂不支持进入！");
    }
}
```
