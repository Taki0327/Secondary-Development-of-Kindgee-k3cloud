...
> 与插件类似，创建类库（金蝶云星空8.2版本安全更新后要求该类库名称必须为 .WebAPI 结尾，其命名空间等需同步修改，否则会报403错误）后将其发布在网站bin目录下；发布时同插件一样打包。
>
...
if (resultType == 1)
{
    ...
    var data = client.Execute<JObject>("Kingdee.WebAPI.Test.ExecuteSelectAll,Kingdee.WebAPI", new object[] { sJson }).ToString();
}
```
...
{
    //后调用接口
    result = InvokeHelper.AbstractWebApiBusinessService("Kingdee.WebAPI.Test.ExecuteSelectAll,Kingdee.WebAPI", null);
}
else
{
